...
def Bar_csv_process(in_csv,barXML_read,ver_hor,row_col,onet):
	try:
		...
		yPoint_values = np.interp(lines_and_values, (lines_and_values.min(), lines_and_values.max()), (0, 2000))
...
def gotoxml(mainXMLl,bar_names,yText_list,yText_Pixel,lines_values,yPoint_values,onetime_ani,column_deviders):
	...
	def add_bar_b(main_gp,bar_text_b,yText_list,yText_Pixel):
		...
		bar_text_b.find('.//parameter[@name="Y"]').attrib['value'] = yText_Pixel
...
def add_bar(mainXML,main_gp,bar_gp,bar_names,line_vals,yPoint_values,pubset_pos,pubset_col,xpos,onetime_ani):
	...
	bar_gp.find('./parameter/parameter/parameter/parameter[@name="X"]').attrib['value'] = xpos
